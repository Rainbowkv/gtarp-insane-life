...
local function SaveStashItems(stashId, items)
	...
	MySQL.single('SELECT stash FROM stashitems WHERE stash = ?', {stashId}, function(result)
        if result then
            -- 记录已存在，执行更新
            MySQL.update('UPDATE stashitems SET items = ? WHERE stash = ?', {encodedItems, stashId})
        else
            -- 记录不存在，插入新数据
            MySQL.insert('INSERT INTO stashitems (stash, items) VALUES (?, ?)', {stashId, encodedItems})
        end
    end)
...
local function SaveOwnedVehicleItems(plate, items)
	...
	-- rb_code
	local encodedItems = json.encode(items)
	MySQL.single('SELECT plate FROM trunkitems WHERE plate = ?', {plate}, function(result)
        if result then
            -- 记录已存在，执行更新
            MySQL.update('UPDATE trunkitems SET items = ? WHERE plate = ?', {encodedItems, plate})
        else
            -- 记录不存在，插入新数据
            MySQL.insert('INSERT INTO trunkitems (plate, items) VALUES (?, ?)', {plate, encodedItems})
        end
    end)
...
local function SaveOwnedGloveboxItems(plate, items)
    ...
    MySQL.single('SELECT plate FROM gloveboxitems WHERE plate = ?', {plate}, function(result)
        if result then
            -- 记录已存在，执行更新
            MySQL.update('UPDATE gloveboxitems SET items = ? WHERE plate = ?', {encodedItems, plate})
        else
            -- 记录不存在，插入新数据
            MySQL.insert('INSERT INTO gloveboxitems (plate, items) VALUES (?, ?)', {plate, encodedItems})
        end
    end)
	...
	if name and id then
		...
		if name == "stash" then
			...
			if Stashes[id] and Stashes[id].isOpen then
				...
				TriggerClientEvent('qb-inventory:client:SetCurrentStash', src, id)
			end
		...
		TriggerClientEvent("qb-inventory:client:OpenInventory", src, {}, QBPlayer.PlayerData.items, secondInv)
	else
		TriggerClientEvent("qb-inventory:client:OpenInventory", src, {}, QBPlayer.PlayerData.items)
	end
end
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info, 'qb-inventory:server:SetInventoryData', fromItemData["created"])
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				...
				local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
                AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info, 'qb-inventory:server:SetInventoryData', itemInfo["created"])
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
                            Player.Functions.RemoveItem(toItemData.name, toAmount, toSlot)
                            AddToTrunk(plate, fromSlot, toSlot, itemInfo["name"], toAmount, toItemData.info, 'qb-inventory:server:SetInventoryData', itemInfo["created"])
				...
				itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
                AddToTrunk(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info, 'qb-inventory:server:SetInventoryData', itemInfo["created"])
			end
		else
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
                            Player.Functions.RemoveItem(toItemData.name, toAmount, toSlot)
                            AddToGlovebox(plate, fromSlot, toSlot, itemInfo["name"], toAmount, toItemData.info, 'qb-inventory:server:SetInventoryData', itemInfo["created"])
				...
				itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
                AddToGlovebox(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info, 'qb-inventory:server:SetInventoryData', itemInfo["created"])
			end
		else
            QBCore.Functions.Notify(src, Lang:t("notify.itemexist"), "error")
		end
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
                            Player.Functions.RemoveItem(toItemData.name, toAmount, toSlot)
                            AddToStash(stashId, fromSlot, toSlot, itemInfo["name"], toAmount, toItemData.info, 'qb-inventory:server:SetInventoryData', itemInfo["created"])
				...
				itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
                AddToStash(stashId, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info, 'qb-inventory:server:SetInventoryData', itemInfo["created"])
			end
		else
            QBCore.Functions.Notify(src, Lang:t("notify.itemexist"), "error")
		end
	...
	elseif fromInventory == "attachment_crafting" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
                AddToDrop(toInventory, toSlot, itemInfo["name"], fromAmount, fromItemData.info, 'qb-inventory:server:SetInventoryData', itemInfo["created"])
				if itemInfo["name"] == "radio" then
					TriggerClientEvent('Radio.Set', src, false)
				end
			end
...
RegisterServerEvent("qb-inventory:server:GiveItem", function(target, name, amount, slot)
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info, "qb-inventory:server:GiveItem", item.created) then
				...
				AddItem(src, item.name, amount, item.slot, item.info, "qb-inventory:server:GiveItem", item.created)
	...
	for _, v in pairs(players) do
		...
		if dist < 3.0 then
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>姓:</strong> {1} <br><strong>名:</strong> {2} <br><strong>领证日期:</strong> {3} <br><strong>驾照类型:</strong> {4}</div></div>',
					args = {
						"驾照",
						item.info.firstname,
						item.info.lastname,
...
CreateUsableItem("id_card", function(source, item)
	...
	for _, v in pairs(players) do
		...
		if dist < 3.0 then
			...
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>公民ID:</strong> {1} <br><strong>名:</strong> {2} <br><strong>姓:</strong> {3} <br><strong>生日:</strong> {4} <br><strong>性别:</strong> {5} <br><strong>国籍:</strong> {6}</div></div>',
					args = {
						"公民身份证",
						item.info.citizenid,
						item.info.firstname,
