...
QBCore.Functions.CreateCallback('qb-occasions:server:checkVehicleLimited', function(source, cb, veh)
    if exports['rb-donator']:IsDonatorVeh(veh) or exports['rb-donator']:IsCustomVeh(veh) then
        TriggerClientEvent('QBCore:Notify', src, '该车需要联系管理组进行交易', 'error')
        cb(false)
    else
        cb(true)
    end
...
RegisterNetEvent('qb-occasions:server:sellVehicle', function(vehiclePrice, vehicleData)
    local src = source
    if exports['rb-donator']:IsDonatorVeh(vehicleData.model) or exports['rb-donator']:IsCustomVeh(vehicleData.model) then  -- rb_code
        exports['external_bansystem']:SystemBanPlayer(src, "检测到二手车交易市场绕过限制直接交易赞助或定制车行为", nil)
        -- TriggerClientEvent('QBCore:Notify', src, '检测到二手车交易市场绕过限制直接交易赞助或定制车行为', 'error')
        return
    end
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-occasions:server:buyVehicle', function(vehicleData)
    ...
    if result[1] and next(result[1]) then
        if Player.PlayerData.money.bank >= result[1].price then
            ...
            local NewPrice = math.ceil((result[1].price / 100) * Config.newPriceRatio)
            ...
            if SellerData then
                ...
                exports["qb-banking"]:AddMoney(Config.usedCarDealerAccout, math.ceil((result[1].price / 100) * Config.usedCarDealerRatio))  -- 二手车商的佣金
            else
            ...
            -- })
            local phoneNumber = exports["lb-phone"]:GetEquippedPhoneNumber(SellerCitizenId)
            ...
            local mailData = {
                ...
                message = Lang:t('mail.message', {
                    value = NewPrice,
                    value2 = QBCore.Shared.Vehicles[result[1].model].name
                })
            }
