...
function OpenMenu(isPedMenu, backEvent, menuType, menuData)
    ...
    if menuType == "default" then
        local header = "购买服装 - $" .. Config.ClothingCost
        if isPedMenu then
            header = "更换服装"
        end
        menuItems[#menuItems + 1] = {
            header = "服装店选项",
            icon = "fas fa-shirt",
            isMenuHeader = true -- 设置为true以使其成为不可点击的标题
        }
        menuItems[#menuItems + 1] = {
            header = header,
            txt = "从各种各样的服装中挑选",
    ...
    clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            if zoneName == 'clothing' then
                exports['qb-core']:DrawText('[E] 服装店')
            elseif zoneName == 'barber' then
                exports['qb-core']:DrawText('[E] 理发店')
            elseif zoneName == 'tattoo' then
                exports['qb-core']:DrawText('[E] 纹身店')
            elseif zoneName == 'surgeon' then
                exports['qb-core']:DrawText('[E] 整形')
            end
        else
            inZone = false
