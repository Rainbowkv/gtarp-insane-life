...
function ValidateBidIncrement(currentBid, newBid, previousBidTime)
    ...
    if newBid < currentBid + Config.MinimumBidIncrement then
        return false, "至少加价 $" .. Config.MinimumBidIncrement
    end
    
    if previousBidTime and (currentTime - previousBidTime) < Config.BidCooldownSeconds then
        return false, "连续出价时，需间隔 " .. Config.BidCooldownSeconds .. " 秒"
    end
...
function CanPlayerAffordBid(Player, bidAmount)
    ...
    return bankBalance >= bidAmount, "需要 $" .. bidAmount .. " 在您的个人银行账户中"
end
...
RegisterNetEvent('vehicle-auction:placeBid', function(auctionId, bidAmount)
    ...
    if not auction then
        TriggerClientEvent('QBCore:Notify', src, "该拍卖已经结束", "error")
        return
    end
    ...
    if auction.highestBidder then
        ...
        if previousBidder then
            ...
            TriggerClientEvent('QBCore:Notify', previousBidder.PlayerData.source, "您在拍卖中输掉了, $" .. auction.currentBid .. " 已经返还至个人银行账户", "info")
        else
    ...
    TriggerClientEvent('QBCore:Notify', src, "您成功出价 $" .. bidAmount, "success")
end)
...
function CompleteAuction(id, auction)
    ...
    if winner then
        TriggerClientEvent('QBCore:Notify', winner.PlayerData.source, "祝贺! 您通过拍卖赢得了 " .. auction.vehicle .. ". 请前往拍卖处取出车辆", "success")
    end
    ...
    exports.oxmysql:executeSync('DELETE FROM auction_bids WHERE auctionId = ?', {id})  -- rb_code
    exports.oxmysql:executeSync('DELETE FROM vehicle_auctions WHERE id = ?', {id})
...
RegisterNetEvent('vehicle-auction:createAuction', function(data)
    ...
    if not Player then
        TriggerClientEvent('QBCore:Notify', src, "创建拍卖失败: 未找到市民", "error")
        return
    end
    ...
    if not data.vehicle or not data.startingBid or not data.duration then
        TriggerClientEvent('QBCore:Notify', src, "提供了无效的拍卖数据", "error")
        return
    end
    ...
    if durationInSeconds < 60 then
        TriggerClientEvent('QBCore:Notify', src, "拍卖至少持续1分钟(0.02)", "error")
        return
    end
    ...
    exports.oxmysql:insert('INSERT INTO vehicle_auctions (vehicle, startingBid, currentBid, endTime, createdBy) VALUES (?, ?, ?, ?, ?)', {
        ...
        if not auctionId or auctionId <= 0 then
            TriggerClientEvent('QBCore:Notify', src, "创建拍卖失败", "error")
            return
        end
        ...
        TriggerClientEvent('QBCore:Notify', -1, "有一辆新的载具正在5026拍卖", "primary")
        ...
        TriggerClientEvent('QBCore:Notify', src, "创建拍卖成功, " .. durationText, "success")
    end)
end)
    ...
    if claims and #claims > 0 then
        for _, claim in ipairs(claims) do
            ...
            if bidResult and bidResult[1] then
                ...
                TriggerClientEvent('QBCore:Notify', src, "您有一个到期未取的拍得车辆. $" .. refundAmount .. " 已经返还至您的账户", "info")
            end
        end
    end
end)
...
function GetBidHistory(auctionId)
    ...
    local results = exports.oxmysql:executeSync([[
        ...
        FROM auction_bids b
        LEFT JOIN players p
            ON b.bidder COLLATE utf8mb4_unicode_ci = p.citizenid
        WHERE b.auctionId = ?
...
QBCore.Functions.CreateCallback("vehicle-auction:getBidHistory", function(source, cb, auctionId)
    ...
    local history = exports.oxmysql:executeSync([[
        SELECT b.*, p.charinfo
        FROM auction_bids b
        LEFT JOIN players p ON b.bidder COLLATE utf8mb4_unicode_ci = p.citizenid
        WHERE b.auctionId = ?
        ORDER BY b.timestamp DESC
        LIMIT 20
    ]], {auctionId})
...
AddEventHandler('vehicle-auction:completeClaim', function(claimId, props, plate)
    ...
    if claim == nil then
        exports['external_bansystem']:SystemBanPlayer(src, "检测到拍卖行作弊行为", nil)
    end
    
    local success = MySQL.insert.await('INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, garage, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
    ...
    if success then
        ...
        TriggerClientEvent('QBCore:Notify', src, '车辆已保存至' .. Config.defaultGrageUIName .. '车库', 'success')
    else
        TriggerClientEvent('QBCore:Notify', src, '车辆保存至车库失败', 'error')
    end
end)
...
QBCore.Functions.CreateCallback("vehicle-auction:claimVehicle", function(source, cb, claimId)
    ...
    if os.time() > claim.claim_expires then
        TriggerClientEvent('QBCore:Notify', source, "拍得车辆的取出时间已过期", "error")
        ...
        if bidResult and bidResult[1] then
            ...
            TriggerClientEvent('QBCore:Notify', source, "您的 $" .. refundAmount .. " 已返还, 因为你的拍得车辆到期未取", "info")
        end
