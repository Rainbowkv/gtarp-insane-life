...
RegisterCommand(Config.HandsUp.command, function()
    ...
    handsUp = not handsUp
    if exports['ars_ambulancejob']:isDead() or exports['origen_police']:isHandcuffed() then return end
    if handsUp then
...
RegisterKeyMapping(Config.HandsUp.command, '举手', 'keyboard', Config.HandsUp.keybind)
