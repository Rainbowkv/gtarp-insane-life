...
RegisterKeyMapping('createscene', '创建场景文字', 'keyboard', Config.CreateSceneKey)
RegisterKeyMapping('deletescene', '删除场景文字', 'keyboard', Config.DeleteSceneKey)
    ...
    if creationLaser then
        CreateThread(function()
            while creationLaser do
                local hit, coords = DrawLaser('按 ~g~E~w~ 放置文字\n按 ~g~G~w~ 再次编辑', {r = 2, g = 241, b = 181, a = 200})
    ...
    if deletionLaser then
        CreateThread(function()
            while deletionLaser do
                local hit, coords = DrawLaser('按 ~r~E~w~ 删除文字\n按 ~r~G~w~ 取消操作', {r = 255, g = 0, b = 0, a = 200})
...
function DrawLaser(message, color)
    ...
    Draw2DText(message, 0, {255, 255, 255}, 0.4, 0.43, 0.888 + 0.025)
