...
local function CreateBankStatement(playerId, account, amount, reason, statementType, accountType)
    ...
    if accountType == 'player' or accountType == 'shared' then
        if accountType == 'player' then account = '个人' end
...
RegisterNetEvent("qb-banking:server:CreateBankStatement", function(playerId, account, amount, reason, statementType, accountType)
...
local function AddMoney(accountName, amount, reason)
    if not reason then reason = '未填理由' end
    local newStatement = {
        amount = amount,
        reason = reason,
        date = os.time() * 1000,
...
QBCore.Functions.CreateCallback('qb-banking:server:openBank', function(source, cb)
    ...
    if gang.label ~= 'none' and not Accounts[gang.label] then CreateGangAccount(gang.label, 0) end
    accounts[#accounts + 1] = { account_name = '个人', account_type = '个人', account_balance = Player.PlayerData.money.bank }
    statements['个人'] = Statements[citizenid] and Statements[citizenid]['个人'] or {}
    for accountName, accountInfo in pairs(Accounts) do
        ...
        if (accountName == job.name and job.isboss) or (accountName == gang.label and gang.isboss) then
            accounts[#accounts + 1] = accountInfo
            if Statements[accountName] then statements[accountName] = Statements[accountName] end
        end
    end
...
QBCore.Functions.CreateCallback('qb-banking:server:withdraw', function(source, cb, data)
    ...
    if accountName == '个人' then
        ...
        if not CreateBankStatement(src, '个人', withdrawAmount, reason, 'withdraw', 'player') then return cb({ success = false, message = Lang:t('error.error') }) end
    ...
    if Accounts[accountName] then
        ...
        if Accounts[accountName].account_type == 'gang' and gang.label ~= accountName and not gang.isboss then return cb({ success = false, message = Lang:t('error.access') }) end
...
QBCore.Functions.CreateCallback('qb-banking:server:deposit', function(source, cb, data)
    ...
    if accountName == '个人' then
        ...
        if not CreateBankStatement(src, '个人', depositAmount, reason, 'deposit', 'player') then return cb({ success = false, message = Lang:t('error.error') }) end
...
QBCore.Functions.CreateCallback('qb-banking:server:internalTransfer', function(source, cb, data)
    ...
    if fromAccountName == '个人' then
        ...
        if not CreateBankStatement(src, '个人', transferAmount, reason, 'withdraw', 'player') then return cb({ success = false, message = Lang:t('error.error') }) end
        cb({ success = true, message = Lang:t('success.transfer') })
    elseif toAccountName == '个人' then
        ...
        if Accounts[fromAccountName].account_type == 'gang' and gang.label ~= fromAccountName and not gang.isboss then return cb({ success = false, message = Lang:t('error.access') }) end
        ...
        Player.Functions.AddMoney('bank', transferAmount, reason)
        if not CreateBankStatement(src, '个人', transferAmount, reason, 'deposit', 'player') then return cb({ success = false, message = Lang:t('error.error') }) end
        cb({ success = true, message = Lang:t('success.transfer') })
    else
        if Accounts[fromAccountName].account_type == 'job' and job.name ~= fromAccountName and not job.isboss then return cb({ success = false, message = Lang:t('error.access') }) end
        if Accounts[fromAccountName].account_type == 'gang' and gang.label ~= fromAccountName and not gang.isboss then return cb({ success = false, message = Lang:t('error.access') }) end
...
QBCore.Functions.CreateCallback('qb-banking:server:externalTransfer', function(source, cb, data)
    ...
    if fromAccountName == '个人' then
        ...
        if not CreateBankStatement(src, '个人', transferAmount, reason, 'withdraw', 'player') then return cb({ success = false, message = Lang:t('error.error') }) end
        if not CreateBankStatement(toPlayer.PlayerData.source, '个人', transferAmount, reason, 'deposit', 'player') then return cb({ success = false, message = Lang:t('error.error') }) end
        cb({ success = true, message = Lang:t('success.transfer') })
    else
        if Accounts[fromAccountName].account_type == 'job' and job.name ~= fromAccountName and not job.isboss then return cb({ success = false, message = Lang:t('error.access') }) end
        if Accounts[fromAccountName].account_type == 'gang' and gang.label ~= fromAccountName and not gang.isboss then return cb({ success = false, message = Lang:t('error.access') }) end
...
QBCore.Functions.CreateCallback('qb-banking:server:openAccount', function(source, cb, data)
    ...
    if not CreateBankStatement(src, '个人', initialAmount, 'Initial deposit for ' .. accountName, 'withdraw', 'player') then return cb({ success = false, message = Lang:t('error.error') }) end
...
CreateThread(function()
    ...
    for _, statement in ipairs(statements) do
        if statement.account_name == '个人' then
...
QBCore.Commands.Add('givecash', 'Give Cash', { { name = 'id', help = 'Player ID' }, { name = 'amount', help = 'Amount' } }, true, function(source, args)
    ...
    if not target then return TriggerClientEvent('QBCore:Notify', src, "附近没有用户", 'error') end
    local targetPed = GetPlayerPed(tonumber(args[1]))
    local targetCoords = GetEntityCoords(targetPed)
    local amount = tonumber(args[2])
    if not amount then return TriggerClientEvent('QBCore:Notify', src, "没有金额", 'error') end
    if amount <= 0 then return TriggerClientEvent('QBCore:Notify', src, "错误金额", 'error') end
    if #(playerCoords - targetCoords) > 5 then return TriggerClientEvent('QBCore:Notify', src, "太远了", 'error') end
    if Player.PlayerData.money.cash < amount then return TriggerClientEvent('QBCore:Notify', src, "没有足够现金", 'error') end
    Player.Functions.RemoveMoney('cash', amount, 'cash transfer')
    target.Functions.AddMoney('cash', amount, 'cash transfer')
    TriggerClientEvent('QBCore:Notify', src, "给予 " .. amount .. "$", 'success')
    TriggerClientEvent('QBCore:Notify', target.PlayerData.source, "收到 " .. amount .. "$", 'success')
end)
