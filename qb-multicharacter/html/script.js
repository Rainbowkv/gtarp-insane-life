                    ...
                    setTimeout(function(){
                        ...
                        $(".title-screen").fadeIn(0, function() {
                            setTimeout(function() {
                                ...
                                $('.jugadores-on').html(' ' + data.players + ' 国内市民');
                            }, 1000);
...
function setupCharInfo(cData) {
    if (cData == 'empty') {
        $('.character-info-valid').html('<span id="no-char">进Kook社区面试<br><br>以获得创建二设的机会</span>');
    } else {
        var gender = "男"
        if (cData.charinfo.gender == 1) { gender = "女" }
        ...
        '<div class="character-info-box"><span id="info-label">生日: </span><span class="char-info-js">'+cData.charinfo.birthdate+'</span></div>' +
        ...
        '<div class="character-info-box"><span id="info-label">银行余额: </span><span class="char-info-js">&#36; '+cData.money.bank+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">电话号码: </span><span class="char-info-js">'+cData.charinfo.phone+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">银行账号: </span><span class="char-info-js">'+cData.charinfo.account+'</span></div>');
    }
}
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $("#play-text").html('<i class="fa fa-sign-in" aria-hidden="true"></i> 回国');
            $("#delete-text").html('<i class="fa fa-trash" aria-hidden="true"></i> 删除');
            $("#play").css({"display":"block"});
        ...
        if ((selectedChar).data('cid') == "") {
            $(selectedChar).addClass("char-selected");
            setupCharInfo('empty')
            $("#play-text").html('<i class="fa fa-plus" aria-hidden="true"></i> 注册');
            $("#play").css({"display":"block"});
            $("#delete").css({"display":"none"});
            $.post('https://qb-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
...
function setCharactersList() {
    var htmlResult = '<div class="character-list-header"><p>我的角色</p></div>'
    for (let i = 1; i <= NChar; i++) {
        htmlResult += '<div class="character" id="char-'+ i +'" data-cid=""><span id="slot-name">空<span id="cid"></span></span></div>'
    }
    htmlResult += '<div class="character-btn" id="play"><p id="play-text">选择角色</p></div><div class="character-btn" id="delete"><p id="delete-text">选择角色</p></div>'
